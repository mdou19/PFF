            ...
            <div class="flex flex-1  flex-col md:flex-row lg:flex-row mx-2">
                <div class="mb-2 border-solid border-gray-300 rounded border shadow-sm w-full">
                    <div class="bg-gray-200 px-2 py-3 border-solid border-gray-200 border-b">
                       GESTION DES PARTICIPATION D'EVENEMENT
                    </div>
                    <div class="p-3">
                        <table class="table-responsive w-full rounded">
                            <thead>
                              <tr>
                                <th class="border w-1/7 px-4 py-2">PRENOM</th>
                                <th class="border w-1/5 px-4 py-2">NOM</th>
                                <th class="border w-1/5 px-4 py-2">TELEPHONE</th>
                                <th class="border w-1/5 px-4 py-2">EMAIL</th>
                                {{-- <th class="border w-1/5 px-4 py-2">NOM D'EVENEMENT</th> --}}
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="border px-4 py-2">{{$reservation_event->prenom}}</td>
                                    <td class="border px-4 py-2">{{$reservation_event->nom}}</td>
                                    <td class="border px-4 py-2">{{$reservation_event->telephone}}</td>
                                    <td class="border px-4 py-2">{{$reservation_event->email}}</td>
                                    {{-- <td class="border px-4 py-2">{{$reservation_event->evenement->nom_evenement}}</td> --}}
                                    <td class="border px-4 py-2">
                                        <form method="POST" action="{{route('participe.destroy', $reservation_event->id)}}" class="bg-teal-300 cursor-pointer rounded p-1 mx-1 text-red-500">
                                            {{ method_field('DELETE') }}
                                            {{ csrf_field() }}
                                            <button type="submit" class="btn btn-danger btn-sm" title="Delete departement" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fas fa-trash"></i> </button>
                                        </form>
                                    </td>
                                </tr>
                                @endforeach
